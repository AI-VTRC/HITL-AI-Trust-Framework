...
def update_datetime_dropdown(selected_folder):
    if selected_folder:
        datetime_options = [{"label": entry["current_datetime"], "value": entry["current_datetime"]} for entry in
...
def update_report_dropdown(selected_folder, selected_datetime):
    if selected_folder and selected_datetime:
        report_options = [{"label": entry["report_json"], "value": entry["report_json"]} for entry in
                          data[selected_folder] if entry["current_datetime"] == selected_datetime]
...
def update_content(selected_folder, selected_datetime, selected_report, image_index):
    ...
    image_sources = [f"/results/{selected_folder}/{selected_datetime}/Car{i + 1}_frame_{image_index}_with_boxes.jpg" for
                     i in range(4)]
